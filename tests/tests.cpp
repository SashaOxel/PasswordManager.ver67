...
static void testGenerator() {
...

    {
        GenOptions o;
        o.length = 12;
        check(generatePassword(o).size() == 12, "Стандартные настройки: длина пароля равна 12");
    }
    {
        GenOptions o;
        o.length = 32;
        check(generatePassword(o).size() == 32, "Длина пароля 32: результат содержит ровно 32 символа");
    }
    {
        GenOptions o;
        o.length = 20; o.useUpper = false; o.useLower = true;
        o.useLetters = true; o.useDigits = false; o.useSymbols = false;
        std::string p = generatePassword(o);
        check(p.size() == 20 && allIn(p, LOWER), "Только строчные буквы: все символы пароля строчные");
    }
    {
        GenOptions o;
        o.length = 20; o.useUpper = true; o.useLower = false;
        o.useLetters = true; o.useDigits = false; o.useSymbols = false;
        std::string p = generatePassword(o);
        check(p.size() == 20 && allIn(p, UPPER), "Только заглавные буквы: все символы пароля заглавные");
    }
    {
        GenOptions o;
        o.length = 20; o.useLetters = false; o.useUpper = false;
        o.useLower = false; o.useDigits = true; o.useSymbols = false;
        std::string p = generatePassword(o);
        check(p.size() == 20 && allIn(p, DIGITS), "Только цифры: все символы пароля цифры");
    }
    {
        GenOptions o;
        o.length = 20; o.useLetters = false; o.useUpper = false;
        o.useLower = false; o.useDigits = false; o.useSymbols = true;
        std::string p = generatePassword(o);
        check(p.size() == 20 && allIn(p, SYMBOLS), "Только спецсимволы: все символы пароля специальные");
    }
    {
        GenOptions o;
        o.length = 50; o.useLetters = true; o.useUpper = true;
        o.useLower = true; o.useDigits = true; o.useSymbols = true;
        std::string p = generatePassword(o);
    ...
    {
        GenOptions o;
        o.useLetters = false; o.useUpper = false; o.useLower = false;
        o.useDigits = false; o.useSymbols = false;
        check(!generatePassword(o).empty(), "Ничего не выбрано: функция возвращает сообщение об ошибке");
    }
    {
        GenOptions o;
        o.length = 16;
